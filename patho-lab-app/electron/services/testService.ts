    ...
    if (existingMap.has(param.parameter_code)) {
      ...
      run(`
        ...
        SET parameter_name = ?, data_type = ?, unit = ?, decimal_precision = ?, 
            display_order = ?, is_mandatory = ?, is_header = ?, formula = ?, parent_id = ?
        WHERE id = ?
      `, [
        param.parameter_name, param.data_type, param.unit, param.decimal_precision,
        param.display_order, param.is_mandatory, param.is_header || 0, param.formula, param.parent_id || null, existing.id
      ]);
    } else {
      // Insert
      run(`
      ...
      `, [
        ...
        param.unit, param.decimal_precision, param.display_order, param.is_mandatory, param.is_header || 0, param.formula, param.parent_id || null
      ]);
    }
  }
